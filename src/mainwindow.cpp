...
//更新游戏用时
void Minesweeper::on_updateTimer()
{
    mBlock->mTime++;
    update();
}
...
void Minesweeper::paintEvent(QPaintEvent *event)
{
    ...
    //加载位图
    QPixmap bmpblocks("/home/uranus/Desktop/Minesweeper/resource/blocks.bmp");//方块
    QPixmap bmpblast("/home/uranus/Desktop/Minesweeper/resource/boomblast.bmp");//雷爆炸
    QPixmap bmpnumber("/home/uranus/Desktop/Minesweeper/resource/number.bmp");//计数用红色数字
    QPixmap bmpfaces("/home/uranus/Desktop/Minesweeper/resource/faces.bmp");//表情
    QPixmap bmpflagerror("/home/uranus/Desktop/Minesweeper/resource/flagerror.bmp");//错误标记
    QPixmap bmpboom2("/home/uranus/Desktop/Minesweeper/resource/Mine0.bmp");
    QPixmap bmpboom3("/home/uranus/Desktop/Minesweeper/resource/Mine1.bmp");
   //绘制雷区
    for(int rowBlock=0;rowBlock<mBlock->mRow;rowBlock++)
    {
        for(int colBlock=0;colBlock<mBlock->mCol;colBlock++)
        {
            ...
            case BS_BOMB_LIVE://未挖的雷
                ...
                else if(mBlock->boomtyple==Typle_2)
                    painter->drawPixmap(colBlock*BLOCK_WIDTH,rowBlock*BLOCK_HEIGHT+BOARD_HEIGHT,bmpboom2,0,0,BLOCK_WIDTH,BLOCK_HEIGHT);
                else if(mBlock->boomtyple==Typle_3)
                    painter->drawPixmap(colBlock*BLOCK_WIDTH,rowBlock*BLOCK_HEIGHT+BOARD_HEIGHT,bmpboom3,0,0,BLOCK_WIDTH,BLOCK_HEIGHT);
                break;
            default:
                break;
            }
...
void Minesweeper::on_actionReStart_triggered()
{
    mBlock->Restart();
    update();
}
//初级
void Minesweeper::on_actionPrimary_triggered()
{
    mBlock->Create(9,9,10);
    setFixedSize(mBlock->mCol*BLOCK_WIDTH,mBlock->mRow*BLOCK_HEIGHT+BOARD_HEIGHT);
}
//中级
void Minesweeper::on_actionInternediate_triggered()
{
    mBlock->Create(16,16,40);
    setFixedSize(mBlock->mCol*BLOCK_WIDTH,mBlock->mRow*BLOCK_HEIGHT+BOARD_HEIGHT);
}
//高级
void Minesweeper::on_actionHigh_triggered()
{
    mBlock->Create(16,30,99);
    setFixedSize(mBlock->mCol*BLOCK_WIDTH,mBlock->mRow*BLOCK_HEIGHT+BOARD_HEIGHT);
}
//用户自定义难度
void Minesweeper::on_actionUser_Define_triggered()
{
...
//退出
void Minesweeper::on_actionExit_triggered()
{
    QApplication::quit();
}

void Minesweeper::on_actionHow_to_play_triggered()
{
    QMessageBox message(QMessageBox::NoIcon, "扫雷规则", "扫雷游戏目标是在最短的时间内根据点击格子出现的数字找出所有非雷格子，同时避免踩雷，踩到一个雷即全盘皆输。\n点开的数字是几，则说明该数字旁边的8个位置中有几个雷。");
    message.exec();
}

void Minesweeper::on_select_boom_1_triggered()
{
    mBlock->boomtyple = Typle_1;
}

void Minesweeper::on_select_boom_2_triggered()
{
    mBlock->boomtyple = Typle_2;
}

void Minesweeper::on_select_boom_3_triggered()
{
    mBlock->boomtyple = Typle_3;
}
